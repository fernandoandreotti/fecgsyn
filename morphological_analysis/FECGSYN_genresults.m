...
BINS = 200;                 % number of bins used for template generation
ch = [1 8 11 22 25 32];     % relevant for ICA/PCA
...
morph_ica = zeros(length(fls_orig),3);
morph_tspca = zeros(length(fls_orig),3);
morph_kf = zeros(length(fls_orig),3);
