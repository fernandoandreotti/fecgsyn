...
switch result
    ...
    otherwise % Fernando loves having to flip slashes
        if isunix
            path = '/media/fernando/FetalEKG/2014.08_fecgsyn_simulations(3.0)/';
            path2save = '/media/fernando/FetalEKG/2014.08_fecgsyn_simulations(3.0)/extracted_hp3Hz/';
...
% % % 
% % % icamethod = 'FASTICA_DEF'; % can be FASTICA_DEF, FASTICA_SYM or JADEICA
% % % 
% % % % == core function
...
% % %         icasig = FECGSYN_bss_extraction(mixture,icamethod,fs,out.fqrs{1},loopsec,filename);     % extract using IC
% % %         % Calculate quality measures
% % %         qrs = qrs_detect(icasig,TH,REFRAC,fs);
% % %         if isempty(qrs)
% % %             F1= 0;
% % %             RMS = NaN;
% % %             PPV = 0;
...
% % %         icasig = FECGSYN_bss_extraction(mixture,'PCA',fs,out.fqrs{1},loopsec,filename);     % extract using IC
% % %         % Calculate quality measures
% % %         qrs = qrs_detect(icasig,TH,REFRAC,fs);
% % %         if isempty(qrs)
% % %             F1= 0;
...
% % %     stats_struct{k}.stats_ica = stats_ica;
% % %     
...
stats_pca = zeros(length(fls),4);
stats_tsc = zeros(length(fls),4);
stats_tspca = zeros(length(fls),4);
stats_kf = zeros(length(fls),4);
stats_lms = zeros(length(fls),4);
stats_rls = zeros(length(fls),4);
stats_esn = zeros(length(fls),4);
