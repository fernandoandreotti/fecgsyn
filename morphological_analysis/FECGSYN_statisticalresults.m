...
symlist = {'+' 'o' '*' '^' 'x' 's' 'd'}; % symbol selection for plot
snrlist = zeros(7,5,3);
% color selection for plot, different colors for each case
snrlist(1,:,:) = [0.560784313725490 0.764705882352941 0.596078431372549;...
...
snrlist(5,:,:) = [[0.800000000000000 0.905882352941177 0.784313725490196] ;...
   [0.635294117647059 0.835294117647059 0.627450980392157];...
   [0.494117647058824 0.776470588235294 0.498039215686275];...
   [0.349019607843137 0.721568627450980 0.372549019607843];...
   [0.133333333333333 0.678431372549020 0.211764705882353]]; % color selection for plot
...
for i1 = size(statsall,1):-1:1
    for i2 = 1:size(statsall,2)
        plot([1:8]+ones(1,8).*count3(i2),reshape(statsall(i1,i2,:),1,8),symlist{i2},'Color',snrlist(i2,i1,:),'MarkerSize',2*i1+3,'MarkerFaceColor',snrlist(i2,i1,:));
    end
end
