function [maxch,fqrs] = FECGSY_blockcompare(residual,fref,fs)
...
% along with this program.  If not, see <http://www.gnu.org/licenses/>.
 ...
 % creating statistics in 1-min blocks
        min = 1;
        maxch = zeros(1,length(residual)/fs/60);
        fqrs_temp = cell(1,length(residual)/fs/60);
        while min <= length(residual)/fs/60;
            F1max = 0;
