...
extremities = (mref <= MWIN | mref >= length(mecg)-MWIN);        % if there are peaks on the border that may lead to error
mref = mref(~extremities);                                          % remove extremity peaks
extremities = (fref <= FWIN | fref >= length(mecg)-FWIN);        % if there are peaks on the border that may lead to error
...
mbeats = 60*fs*length(mref)/length(mecg); % now im bpm
fbeats = 60*fs*length(fref)/length(mecg); % now im bpm

Pmat = sum(mecg(:,mat_sig).^2,2)*(length(mref)/sum(m_nover)); % maternal power (in each channel)
Pmat = Pmat.*(MHR/mbeats);                          % normalized power
Pfet = sum(fecg(:,fet_sig).^2,2)*(length(fref)/sum(f_nover)); % fetal power (in each channel)
Pfet = Pfet.*(FHR/fbeats);                          % normalized power
Pn   = sum(noise(:,noise_sig).^2,2); % noise power (in each channel)
