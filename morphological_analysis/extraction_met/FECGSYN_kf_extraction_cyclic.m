...
%% ===== Looping through cycles =======000
phase = PhaseCalc(peaks,length(ecg)); % overall phase calculation

% dividing cycles
cyclesend = unique([nbCycles:nbCycles:length(peaks) length(peaks)]);
if (cyclesend(end)- cyclesend(end-1) < 10) % if last loop has less than 10
    cyclesend(end-1) = [];  % last two are merged
end
ssamp = 1;
for i = 1:length(cyclesend)
    if i < length(cyclesend)
        [~,endsamp] = max(phase(peaks(cyclesend(i)):peaks(cyclesend(i)+1)));
        endsamp = endsamp + peaks(cyclesend(i));
    else
        endsamp = length(ecg);
    end
    samp2filt = ssamp:endsamp; % creating a list with samples to filter
    ecgtmp = ecg(samp2filt);
    pmaternal=peaks(peaks >= ssamp & peaks<=endsamp)-ssamp+1; % maternal peaks within interval
    %Processing this part
    %% == generate KF's model
    [OptimumParams,phase_tmp,ECGsd,w,wsd] = FECGSYN_kf_ECGmodelling(ecgtmp,pmaternal,nbCycles,fs);
    
    %% == MECG estimation using KF
    % = Kalman Filter Parametrization
    
    p = [0.01 0.001 0.001 1 10 0.00001 10];
    y = [phase_tmp ; ecgtmp];    % state
    ...
    P0 = diag([(2*pi)^2,(10*max(abs(ecgtmp))).^2]); % error covariance matrix
    ...
    % control input
    u = zeros(1,length(ecgtmp));
    
    % = Run KF
    filteredsignalpart = FECGSYN_kf_EKFilter(y,X0,P0,Q,R,Wmean,Vmean,OptimumParams,w,fs,flag,u);    
    
    %Saving this part
    Xhat(:,samp2filt)= filteredsignalpart;
    %Augment offsets
    ssamp = endsamp+1;
