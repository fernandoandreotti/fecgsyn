...
clearvars -except stat stat_nopca
...
hold on
x = ch;
h(1)=shadedErrorBar(x, FASTICA_DEF, {@mean, @(x) std(x)  },{'d--','markerfacecolor',cc(1,:),'color',cc(1,:),'linewidth',LWIDTH},1);
h(2)=shadedErrorBar(x, FASTICA_SYM, {@mean, @(x) std(x)  },{'^--','markerfacecolor',cc(2,:),'color',cc(2,:),'linewidth',LWIDTH},1);
h(3)=shadedErrorBar(x, JADEICA, {@mean, @(x) std(x)  },{'s--','markerfacecolor',cc(3,:),'color',cc(3,:),'linewidth',LWIDTH},1);
h(4)=shadedErrorBar(x, pca, {@mean, @(x) std(x)  },{'o--','markerfacecolor',cc(4,:),'color',cc(4,:),'linewidth',LWIDTH},1);
han = arrayfun(@(x) x.mainLine,h,'UniformOutput',0);
legend([han{:}],'sym. FAST-ICA','defl. FAST-ICA','JADE','PCA')
set(findall(gcf,'type','text'),'fontSize',FONT_SIZE);
set(gca,'FontSize',FONT_SIZE)
set(gca,'XTick',ch)
ylabel('F_1 (in %)')
...
legend('mean ICA (JADE)','median ICA (JADE)','mean ICA (sym. FAST-ICA)','median ICA (sym. FAST-ICA)',...
    'mean ICA (defl. FAST-ICA)','median ICA (defl. FAST-ICA)','mean PCA','median PCA')
set(findall(gcf,'type','text'),'fontSize',FONT_SIZE);
set(gca,'FontSize',FONT_SIZE,'linewidth',LWIDTH)
set(gca,'XTick',ch)
ylabel('F_1 (in %)')
xlabel('                                                                                                 Number of Channels')
box on
