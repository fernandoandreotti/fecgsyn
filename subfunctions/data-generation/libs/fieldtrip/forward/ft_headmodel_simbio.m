...
% fecgsyn v1.3 edit: remove tissue from struct prior to passing
% to ft_datatype_parcellation to suppress dimord warning
% Issue documented at https://mailman.science.ru.nl/pipermail/fieldtrip/2016-July/010693.html
tissue = mesh.tissue;
mesh = rmfield(mesh,'tissue');
mesh = ft_datatype_parcellation(mesh);
mesh.tissue = tissue;
% Backup check to ensure equal number of tissues and elements
if (size(mesh.tissue,1) ~= size(mesh.tet,1))
    error('Mesh has inconsistent number of elements and designated tissues.')
end
% end fecgsyn v1.3 edit
